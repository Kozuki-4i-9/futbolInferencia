...
web = f"https://es.wikipedia.org/wiki/Copa_Mundial_de_F%C3%BAtbol_de_1982"
...
for des0, des1 in enumerate(data_grupo_0[1:]):
    ...
    confirm_grupo = des1.find_elements(By.XPATH, ".//preceding::h4//span[contains(@id, 'Grupo_')]")

    print("data_grupo_10: ", len(data_grupo_11), len(confirm_grupo))
    ...
    for des4 in data_grupo_11:
        ...
        for des5, des6 in enumerate(data_grupo_2):
            if des5==0:
                impresion = des6.find_elements(By.XPATH, ".//a[contains(@title, 'Selección de fútbol de') or @title='Corea del Sur']")
                for des in impresion:
                    lst_col_tr1.append(des.text.strip())
            elif des5==1:
                impresion = des6.find_elements(By.XPATH, ".//b")
                for des in impresion:
                    lst_col_tr1.append(des.text.strip())
            elif des5>1:
        ...
        lst_col_tr1.append(confirm_grupo[-1].text.strip())
...
df_base["year"] = "1982"
df_base.to_csv("grupos_mundiales_1982.csv")