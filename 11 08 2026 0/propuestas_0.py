...
# 11 08 2026
- nada de (formar_dataset_real) llega con parametros de rendimiento extra
- los parametros de rendimiento extra se agregan solo a df2 (fase de grupos del fixtura del mundial jugado ); df1 (partidos reales del mundial jugado (ambas fases))
- los parametros de rendimiento extra se agregan con agregar_features, que depende de (dic_agnos) que depende de partidos de eliminatorias (formar_dataset_real("clasificaciones"))
- con (seleccionar) se lleba la estructura de metricas de rendimiento base de clasificaciones al resto de las fases 
- con (calculo_metricas_0) actualmente se quitan (PJ_0   GF_0   GC_0   D_0) y se agrega (ts_GF_0   ts_GC_0) a (home  PTS_0  PG_0  PP_0  PE_0  D_0  score_0) formando 9 
...
- con (calculo_metricas_0) deberian agregarse n columnas como un lienzo fijo para metricas extra y rellenar de forma opcional las que correspondan a las escogidas por 
